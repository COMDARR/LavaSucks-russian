...
namespace LavaSucks
{
	...
	public class LavaSucks : TerrariaPlugin
	{
		...
		#region Info
		public override string Name => "LavaSucks russian";
		
		public override string Author => "Comdar";
		
		public override string Description => "Автоматически удаляет лаву из адского камня во время добычи.";
		...
		void OnInitialize(EventArgs args)
		{
			...
			Commands.ChatCommands.Add(new Command("lavasucksru.admin.toggle", ToggleLava, "lava")
				{
					HelpText = "Включает/выключает лаву при добыче адского камня. Настройка по умолчанию выключена."
			});
		}
		...
		void ToggleLava(CommandArgs args)
		{
            ...
            args.Player.SendSuccessMessage($"Лава из добычи адского камня теперь {((doesLavaSuck) ? "выключена" : "включена")}!");
		}
	}
}
